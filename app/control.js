...
var _vueAlias = _interopRequireDefault(__webpack_require__(5));
...
var _url = _interopRequireDefault(__webpack_require__(4));
    ...
    transformSong: function (data) {
            ...
            if (line.match("^\\s*$")) // Not sure what this is checking for...
              {
                if (verse_text.lastChild) // && verse_text.lastChild.tagName != 'br')
                  {
                    verse_text.push('---'); // verse_text.appendChild( document.createElementNS('http://www.w3.org/1999/xhtml','hr') );
                  }
              } else {
      ...
      if (order) {
        for (var index in order) {
          ...
          if (verses[index]) {// this._transform_verse(index, verses);
          }
        } // If no order is set, just print the verses by their order in the file

      } else {
        for (var index in verses) {// this._transform_verse(index, verses,);
        }
      }

      this.verses = verses;
      this.selectedVerse = Object.keys(verses)[0];
      this.selectVerse(); //transformed = transformed.replace(empty_verses_regexp,'');
      // return transformed;
    },
    _transform_verse: function (index, verses) {
      // Maybe deprecated, since this is mostly generating markup
      if (index.slice(-8) == '_region2') return;
      ...
      if (!verses[index + '_region2']) {
        for (var index = 0; index < verse.childNodes.length; index++) {
          ...
          if (child.tagName != 'hr') desc.appendChild(child.cloneNode(true));else if (desc.hasChildNodes()) {
        ...
        if (!region2_size) region2_size = 100;
        ...
        while (reg1_part < verse.childNodes.length || reg2_part < region2.childNodes.length) {
          ...
          if (verse.childNodes[reg1_part] && verse.childNodes[reg1_part].tagName == 'br') reg1_part++;
          if (region2.childNodes[reg2_part] && region2.childNodes[reg2_part].tagName == 'br') reg2_part++;

          if ((!verse.childNodes[reg1_part] || verse.childNodes[reg1_part].tagName == 'hr') && (!region2.childNodes[reg2_part] || region2.childNodes[reg2_part].tagName == 'hr')) {
            item.appendChild(desc);
            listbox.appendChild(item);
            item = document.createElement('richlistitem');
            desc = document.createElement('description');
